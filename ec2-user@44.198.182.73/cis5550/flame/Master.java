    ...
    post("/submit", (request,response) -> {
      ...
      try {
        ...
        return sw.toString();
    	  ...
    	  logger.error("The job threw an exception, which was:", e.getCause());
          StringWriter sw = new StringWriter();
          e.getCause().printStackTrace(new PrintWriter(sw));
          response.status(500, "Job threw an exception");
      }
      
      if(flameContext.getOutputCalled()==false) {
    	  response.status(200, "OK");
    	  response.body("No output");
    	  return "No output";
      }
      
      response.body(flameContext.getOutputBody());
      return flameContext.getOutputBody();
    });
